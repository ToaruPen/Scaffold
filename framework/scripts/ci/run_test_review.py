...
def main() -> int:
    ...
    run_dir = (
        repo_root / config.results_dir / config.scope_id / config.run_id / config.engine / _STAGE
    )
    ...
    stage_exit = shared._run_gate(
        repo_root=repo_root,
        gate_script=repo_root / "framework/scripts/gates/validate_test_review.py",
    ...
    metadata = {
        ...
        "configured_model": config.codex_model if config.engine == "codex" else config.claude_model,
        "configured_effort": (
        ...
        "entrypoints": {
            ...
            "test_review_gate_result": shared._relative_path(repo_root, stage_result),
    ...
    if stage_exit == 0 and evidence_exit == 0:
        return 0
    return 2
