...
def _build_result(payload: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    if not passed:
        result["errors"] = [_error("E_PROVIDER_FAILURE", "review-cycle evidence check failed")]
    return result, passed


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate review-cycle evidence contract")
